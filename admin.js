const ADMIN_PASS = 'conchichina2024';
...
function renderAdminList() {
  ...
  if (!adminProducts.length) {
    el.innerHTML = '<p style="color:#888; font-style:italic; text-align:center; padding:20px 0">No hay productos aún.</p>';
    return;
  }
  ...
  if (!filtered.length) {
    el.innerHTML = '<p style="color:#888;font-style:italic;text-align:center;padding:20px 0">No hay productos con ese filtro.</p>';
    return;
  }
...
const DEFAULT_CATEGORIAS = [
  { id: 'pendientes', nombre: 'Pendientes', emoji: '💎' },
  { id: 'anillos', nombre: 'Anillos', emoji: '💍' },
  { id: 'collares', nombre: 'Collares', emoji: '📿' }
];
...
function renderCategoriasAdmin() {
  ...
  if (!cats.length) {
    el.innerHTML = '<p style="color:#888;font-style:italic">No hay categorías.</p>';
  } else {
    el.innerHTML = '<div class="admin-product-list">' +
      cats.map((c, i) => `
...
async function saveBanner() {
  ...
  const config = {
    ...
    texto: msgInput || '💎 Nueva colección de pendientes — ¡Escríbeme!',
    color
  };
